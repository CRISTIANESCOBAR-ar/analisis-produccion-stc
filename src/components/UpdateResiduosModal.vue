...
const fetchStatus = async () => {
  ...
  try {
    ...
    const resp = await fetch(`${API_BASE}/import-status${folderParam}`)
...
const triggerUpdate = async () => {
  ...
  try {
    ...
    const resp = await fetch(`${API_BASE}/import/update-outdated`, {
      method: 'POST',
...
const forceImport = async (item) => {
  ...
  try {
    const resp = await fetch(`${API_BASE}/import/force-table`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
        table: item.table,
