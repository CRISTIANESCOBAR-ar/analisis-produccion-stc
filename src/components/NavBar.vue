<template>
  ...
  <button 
    v-if="!isSidebarOpen"
    @click="openSidebar"
    class="lg:hidden fixed top-4 left-4 z-50 p-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition-all duration-300"
  ...
  <!-- Detector invisible en el borde izquierdo (solo desktop) -->
  <div class="hidden lg:block fixed top-0 left-0 h-full w-12 z-50" @mouseenter="onLeftEdgeEnter"></div>

  <!-- Overlay para cerrar el sidebar en móvil -->
  <div 
    v-if="isSidebarOpen" 
    @click="closeSidebar"
    class="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-[9998] transition-opacity duration-300"
  ></div>
  ...
  <aside 
    ...
    :class="[
      'fixed top-0 left-0 h-full bg-blue-800 text-white transition-all duration-500 ease-in-out',
