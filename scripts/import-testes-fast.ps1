param(
  ...
  [Parameter(Mandatory=$false)][string]$Sheet = 'report2'
)
...
try {
  ...
  $cmds = @(
    ...
    "CREATE TEMP TABLE temp_testes AS SELECT * FROM tb_TESTES WHERE 1=0;",
    ".mode csv",
    ".import $csvPath temp_testes",
    "BEGIN;",
    "DELETE FROM tb_TESTES WHERE DATE(DT_PROD) IN (SELECT DISTINCT DATE(DT_PROD) FROM temp_testes);",
    "INSERT INTO tb_TESTES SELECT * FROM temp_testes;",
    "COMMIT;",
    "DROP TABLE temp_testes;"
  )

  & sqlite3 $SqlitePath @cmds
  Write-Host "Importacion TESTES completada (fast csv, date_delete)." -ForegroundColor Green
  ...
  try {
    ...
    $rows = (& sqlite3 $SqlitePath "SELECT COUNT(*) FROM tb_TESTES;").Trim()
...
VALUES ('tb_TESTES', '$safePath', '$Sheet', '$importDate', '$xlsxLastModified', 'NA', $rows, 'fast_csv')
...
"@
    ...
    Write-Warning "No se pudo actualizar import_control para tb_TESTES: $_"
  }
}
finally {
