...
Write-Host "  EXPORTACION A ACCESS - INICIO" -ForegroundColor Cyan
...
function Measure-AccessStep {
    ...
    try {
        ...
        Write-Host "[OK] COMPLETADO: $Name - $segundos segundos" -ForegroundColor Green
...
Write-Host "Si algo falla, puede restaurar este archivo." -ForegroundColor Yellow
...
Write-Host "" -ForegroundColor Cyan
Write-Host "Verificando existencia de archivos Excel..." -ForegroundColor Cyan
...
if ($archivosNoEncontrados.Count -gt 0) {
    ...
    Write-Host "ADVERTENCIA: $($archivosNoEncontrados.Count) archivo no encontrado" -ForegroundColor Yellow
    Write-Host "El proceso continuara con los archivos disponibles." -ForegroundColor Yellow
}
...
Write-Host "================================================" -ForegroundColor Green
Write-Host "" -ForegroundColor Cyan
Write-Host "Resumen de tiempos:" -ForegroundColor Cyan
$script:timings | Format-Table -AutoSize
$totalSeg = [math]::Round($globalStopwatch.Elapsed.TotalSeconds, 2)
$totalMin = [math]::Round($globalStopwatch.Elapsed.TotalMinutes, 2)
Write-Host "Total: $totalSeg segundos - $totalMin minutos" -ForegroundColor Yellow

# Verificar si hubo errores
$errores = $script:timings | Where-Object { $_.Estado -eq "ERROR" }
if ($errores.Count -gt 0) {
    Write-Host "" -ForegroundColor Red
    Write-Host "[WARN] ADVERTENCIA: Hubo errores en algunos procesos:" -ForegroundColor Red
    $errores | Format-Table -AutoSize
    exit 1
} else {
    Write-Host "" -ForegroundColor Green
    Write-Host "[OK] Todos los procesos completados sin errores" -ForegroundColor Green
    exit 0
}
